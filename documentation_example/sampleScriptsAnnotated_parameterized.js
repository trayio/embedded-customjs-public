/* *****************************************************************************************************
/ Reference : https://tray.io/documentation/embedded/advanced-topics/custom-js/custom-js-intro/ ; 
/ https://tray.io/documentation/embedded/advanced-topics/custom-js/components/ ; and 
/ https://tray.io/documentation/embedded/advanced-topics/custom-js/best-practices/
/ *****************************************************************************************************
...
/* *****************************************************************************************************
// Config Wizard Screen 1, no slots
// This is used as an informational screen explaining the upcoming steps. 
/ ***************************************************************************************************** /*


/* *****************************************************************************************************
// Config Wizard Screen 2, slotExternalId = external_acme_authentication ; slotExternalId = external_salesforce_authentication
// Authentication slots for both "Acme" and Salesforce
/ ***************************************************************************************************** /*


/* *****************************************************************************************************
// Config Wizard Screen 3, slotExternalId = external_salesforce_entity
...
//parameters - this is for a token-based/api key authtication call to a generic http client. This can be set as a static config 
//variable so that it doesn't need to be edited in all locations 
BASE_URL = "https://" //add the base url -- *** can this be a solution level config variable?
ENDPOINT = "" // add the endpoint/ path after the base url *** same as above
...
tray.on('CONFIG_SLOT_MOUNT', async ({ event, previousWizardState }) => {
	...
	const response = await tray.callConnector({
		...
		input: {
			...
			"url": `https://baedxxxx-xxxx-xxxx-xxxx-xxxx68735ac.trayapp.io/objects/${acmeObject}/fields`,
			"reject_unauthorized": true
		}
	})
	.then(response => JSON.parse(JSON.parse(response).response.body));
